...
export function RiskMap({ compact = false, period = "current", onSelectionChange }: { compact?: boolean; period?: DisasterMapPeriod; onSelectionChange?: (event: IndiaDisasterEvent) => void }) {
  ...
  const selectEvent = (event: IndiaDisasterEvent) => { setSelectedId(event.id); onSelectionChange?.(event); };
  ...
  return <div className={cn("relative overflow-hidden rounded-2xl border", compact ? "h-[400px] border-white/10" : "h-[640px] border-slate-200")}>
    <div className="india-map-surface absolute inset-0">
      <svg className="absolute inset-0 h-full w-full" viewBox={indiaMap.viewBox} preserveAspectRatio="xMidYMid meet" role="img" aria-label="India disaster intelligence map with prototype current disaster markers">
        ...
        {selectedLayers.includes("Flood intelligence") && <><path d="M448 220 C470 224 493 237 505 260 L493 288 C474 279 454 266 438 247 Z" fill="rgba(66,145,177,.28)" stroke="#4f94ad" strokeWidth="2" /><path d="M279 474 C296 464 318 467 334 484 L329 516 C310 512 292 501 278 487 Z" fill="rgba(66,145,177,.25)" stroke="#4f94ad" strokeWidth="2" /><text x="445" y="218" fill="#3f8098" fontSize="8" fontWeight="800" letterSpacing="1">FLOOD WATCH</text></>}
        {selectedLayers.includes("Cyclone tracks") && <><path d="M527 357 C505 342 493 320 498 297 C504 271 525 254 551 246" fill="none" stroke="rgba(211,112,47,.22)" strokeWidth="18" strokeLinecap="round" /><path d="M527 357 C505 342 493 320 498 297 C504 271 525 254 551 246" fill="none" stroke="#d47745" strokeWidth="2.5" strokeDasharray="7 6" /><circle cx="527" cy="357" r="7" fill="#e98b42" stroke="white" strokeWidth="2" /><text x="486" y="375" fill="#b7603d" fontSize="8" fontWeight="800" letterSpacing="1">CYCLONE TRACK</text></>}
        {selectedLayers.includes("Earthquake activity") && <><circle cx="315" cy="269" r="25" fill="rgba(229,82,82,.08)" stroke="rgba(229,82,82,.3)" strokeWidth="1.5" /><circle cx="315" cy="269" r="14" fill="rgba(229,82,82,.13)" stroke="#e65353" strokeWidth="2" /><circle cx="315" cy="269" r="4.5" fill="#e65353" stroke="white" strokeWidth="2" /><circle cx="358" cy="285" r="18" fill="rgba(229,82,82,.1)" stroke="rgba(229,82,82,.55)" strokeWidth="1.5" /><circle cx="358" cy="285" r="4" fill="#e65353" stroke="white" strokeWidth="2" /><text x="282" y="242" fill="#c34f4e" fontSize="8" fontWeight="800" letterSpacing="1">SEISMIC ACTIVITY</text></>}
      ...
      {visibleEvents.map((event) => <button key={event.id} onClick={() => selectEvent(event)} className={cn("map-marker absolute z-10", selectedId === event.id && "is-selected")} style={{ top: event.top, left: event.left, ["--marker-color" as string]: event.color }} aria-label={`Select ${event.hazard} in ${event.state}`}><span className="marker-ring" /><span className="marker-dot" /></button>)}
